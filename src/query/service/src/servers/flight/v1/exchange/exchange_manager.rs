...
use super::queries_coordinator::QueriesCoordinator;
use super::queries_coordinator::QueryEvent;
use super::queries_coordinator::create_flight_client;
...
impl DataExchangeManager {
    ...
    pub async fn init_query_env(
            ...
            .await
        {
            ...
            Err(cause) => {
                let _ = handle.transition(QueryEvent::Fail);
                Err(cause)
            }
        }
    }
    ...
    pub fn execute_partial_query(&self, query_id: &str) -> Result<()> {
        ...
        handle.transition(QueryEvent::Start)?;
