...
impl QueryState {
    fn apply(self, event: QueryEvent) -> Option<Self> {
        match (self, event) {
            (QueryState::Created, QueryEvent::InitEnv) => Some(QueryState::EnvInitialized),
            (QueryState::EnvInitialized, QueryEvent::PrepareFragments) => {
                Some(QueryState::FragmentsPrepared)
            }
            (QueryState::EnvInitialized, QueryEvent::Start) => Some(QueryState::Running),
            (QueryState::FragmentsPrepared, QueryEvent::Start) => Some(QueryState::Running),
            (QueryState::EnvInitialized, QueryEvent::BeginFinish)
            | (QueryState::FragmentsPrepared, QueryEvent::BeginFinish)
            | (QueryState::Running, QueryEvent::BeginFinish) => Some(QueryState::Finishing),
            (QueryState::Finishing, QueryEvent::Finish) => Some(QueryState::Finished),
            (_, QueryEvent::Fail) if !self.is_terminal() => Some(QueryState::Failed),
            _ => None,
        }
    }
...
impl QueryHandle {
    fn create(query_id: String) -> Self {
        QueryHandle {
            query_id,
    ...
    pub(crate) fn transition(&self, event: QueryEvent) -> Result<QueryState> {
        let mut state = self.state.lock();
        let from = *state;
        let to = from.apply(event).ok_or_else(|| {
            ErrorCode::Internal(format!(
                "Invalid queries coordinator transition: query_id={}, event={:?}, from={:?}",
                self.query_id, event, from
            ))
        })?;
        ...
        *state = to;
        ...
        info!(
            "queries_coordinator_state_change: query_id={}, event={:?}, from={:?}, to={:?}",
            self.query_id, event, from, to
        );

        Ok(to)
    }

    pub(crate) fn finish(&self, cause: Option<ErrorCode>) {
        match cause {
            Some(_) => {
                let _ = self.transition(QueryEvent::Fail);
            }
            None => {
                let _ = self.transition(QueryEvent::BeginFinish);
                let _ = self.transition(QueryEvent::Finish);
            }
        }
    }
}
...
impl QueryRuntime {
    ...
    pub(crate) fn execute(&self, exchanges: &QueryExchangeRegistry) -> Result<()> {
        ...
        let mut pipelines = Vec::with_capacity(state.fragments.len());

        let mut params = Vec::with_capacity(state.fragments.len());
        for fragment in state.fragments.values() {
            params.push(
                fragment.create_exchange_params(
                    &exchange_info,
                    fragment
                        .pipeline_build_res
                        .as_ref()
        ...
        for ((_, fragment), params) in state.fragments.iter_mut().zip(params) {
...
impl QueriesCoordinator {
    ...
    pub(crate) fn create_query(&self, query_id: String) -> Result<Arc<QueryHandle>> {
        ...
        match queries.entry(query_id.clone()) {
            ...
            Entry::Vacant(v) => {
                let handle = Arc::new(QueryHandle::create(query_id));
...
mod tests {
    ...
    #[test]
    fn query_state_machine_accepts_normal_lifecycle() -> Result<()> {
        let handle = QueryHandle::create("query-1".to_string());

        assert_eq!(*handle.state.lock(), QueryState::Created);
        assert_eq!(
            handle.transition(QueryEvent::InitEnv)?,
            QueryState::EnvInitialized
        );
        assert_eq!(
            handle.transition(QueryEvent::PrepareFragments)?,
            QueryState::FragmentsPrepared
        );
        assert_eq!(handle.transition(QueryEvent::Start)?, QueryState::Running);
        assert_eq!(
            handle.transition(QueryEvent::BeginFinish)?,
            QueryState::Finishing
        );
        assert_eq!(handle.transition(QueryEvent::Finish)?, QueryState::Finished);
        assert_eq!(*handle.state.lock(), QueryState::Finished);

        Ok(())
    }

    #[test]
    fn query_state_machine_rejects_invalid_transition() -> Result<()> {
        let handle = QueryHandle::create("query-1".to_string());

        assert!(handle.transition(QueryEvent::Start).is_err());
        assert_eq!(*handle.state.lock(), QueryState::Created);
    ...
    #[test]
    fn query_state_machine_can_start_after_env_without_local_fragments() -> Result<()> {
        let handle = QueryHandle::create("query-1".to_string());
        ...
        assert_eq!(handle.transition(QueryEvent::Start)?, QueryState::Running);
        assert_eq!(*handle.state.lock(), QueryState::Running);

        Ok(())
    }

    #[test]
    fn query_state_machine_rejects_duplicate_start() -> Result<()> {
        let handle = QueryHandle::create("query-1".to_string());
    ...
    #[test]
    fn query_state_machine_can_fail_before_finish() -> Result<()> {
        let handle = QueryHandle::create("query-1".to_string());

        assert_eq!(handle.transition(QueryEvent::Fail)?, QueryState::Failed);
        assert!(handle.transition(QueryEvent::Finish).is_err());
