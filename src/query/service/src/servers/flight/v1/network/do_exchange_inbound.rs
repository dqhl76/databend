        ...
        *next_sequence += 1;
        ...
        match accepted? {
            ...
            DeliveryOutcome::AllReceiversClosed => {
                // No downstream receiver can accept more data, so tell the sender to stop.
...
impl InboundDestination {
    fn release(&self, terminal: &InboundTerminal) {
        if let Some(cause) = terminal.cause() {
            let mut close_cause = self.close_cause.lock();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_delivery_deduplicates_replayed_data() {
        let channels = NetworkInboundChannelSet::new(1);
        let source =
            NetworkInboundSource::new(&channels, 1024, Duration::ZERO, "test source".to_string());
        ...
        let received = channels.channels[0].recv_raw().await.unwrap().unwrap();
        let QueueItem::RemoteData(received) = received else {
            panic!("New Flight must enqueue remote data")
        };
        ...
        assert!(matches!(
            channels.channels[0].receiver.try_recv(),
    ...
    #[tokio::test]
    async fn test_disconnect_without_retries_propagates_error() {
        ...
        let connection = source.connect(
            runtime,
            ErrorCode::CannotConnectNode("New Flight connection was lost"),
        );
        ...
        channels.channels[0].recv_raw().await.unwrap().unwrap();
        let Err(error) = channels.channels[0].recv_raw().await else {
            panic!("disconnect must fail the inbound channel after queued data is drained")
        };
        assert_eq!(error.code(), ErrorCode::CANNOT_CONNECT_NODE);
    }

    #[tokio::test]
    async fn test_reconnect_invalidates_stale_lease() {
        let runtime = Arc::new(Runtime::with_worker_threads(1, None).unwrap());
        let channels = NetworkInboundChannelSet::new(1);
        let source = NetworkInboundSource::new(
            &channels,
            1024,
